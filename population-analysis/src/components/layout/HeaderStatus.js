...
export default function HeaderStatus({ isTraining, progress, aiModel }) {
  return (
      ...
      {isTraining ? (
        <div className="mt-4 inline-flex flex-col items-center gap-2 bg-yellow-100 text-yellow-800 px-6 py-3 rounded-full">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold">Đang huấn luyện... {progress}%</span>
          </div>
          <div className="w-48 h-2 bg-yellow-200 rounded-full overflow-hidden">
            <div className="h-full bg-yellow-600 transition-all duration-300" style={{ width: `${progress}%` }}></div>
          </div>
        </div>
      ) : aiModel?.isTrained ? (
