...
export function buildForecast(aiModel, currentData, forecastYears, birthRate, deathRate) {
  if (!aiModel || !aiModel.isTrained) return [];
  const data = [];
  let pop = currentData.population;
  let currentBirth = birthRate ?? currentData.birthRate;
  let currentDeath = deathRate ?? currentData.deathRate;
